...
begin # * Filter out poor-performing mice and regenerate order parameters
    ...
    direction of the wavenumber var=:v). One of these will take about 1 minute over 64 cores
    """
    function filter_out(out, newsessions, var = :v, f = sign)
    ...
    Og, Og_h, Og_m, sessionids = filter_out(out, newsessions, :v, sign) # mean order parameter
    GC.gc()
