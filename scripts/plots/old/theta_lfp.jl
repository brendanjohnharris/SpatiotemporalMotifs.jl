...
begin # * Filter out poor-performing mice and regenerate order parameters
    ...
    Og = map(out) do o
        O = map(o) do p
            ...
            x = set(x, Dim{:changetime} => Trial)
            set(x, DimensionalData.format(Trial(trials.hit), lookup(x, Trial)))
        end
    end
    ...
    function toflat(H)
        H = map(H) do h
            map(h) do _h
                ...
                x = ToolsArray(x', (DimensionalData.AnonDim(axes(x, 2)), dims(_h, Trial)))
            end
        end
        H = map(H) do h
            cat(h..., dims = 1)
...
begin # * Save
    addlabels!(fig)
    wsave(plotdir("theta_lfp", "theta_lfp.pdf"), fig)
end
